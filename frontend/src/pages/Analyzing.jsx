      ...
      <div className="flex flex-col items-center justify-center flex-1 px-6">
        <div className="w-full max-w-md">
          <div className="mb-8">
            ...
            {!done && (
              <p className="text-zinc-500 text-sm mt-1">{analysisSteps[currentStep]?.text}...</p>
            )}
          </div>
          ...
          <div className="space-y-2">
            {analysisSteps.map((step, index) => {
              ...
              return (
                <div key={step.id} className={`flex items-center gap-3 py-2 transition-all duration-300 ${isPending ? "opacity-25" : ""}`}>
                  <div className={`w-4 h-4 rounded-sm flex items-center justify-center flex-shrink-0 text-xs border transition-colors ${
                    ...
                    {isCompleted ? "✓" : isCurrent ? <span className="animate-pulse">·</span> : ""}
                  </div>
          ...
          {done && <p className="text-zinc-500 text-xs mt-8 text-center">Redirecting to results...</p>}
        </div>
      </div>
    </main>
  );
