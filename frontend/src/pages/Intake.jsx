...
export default function Intake() {
  ...
  const [formData, setFormData] = useState({
    ...
    sleep_hours: "", stress_level: "", diet_strictness: "",
    dietary_preference: "", primary_goal: "", ideal_physique: "",
  });
      ...
      case 1: return (
        <div className="space-y-4">
          <p className="text-zinc-600 text-xs border-l border-zinc-700 pl-3">
            Additional dimensions are extracted automatically via computer vision analysis of your photos.
          </p>
          <div>
            <label className="text-zinc-500 text-xs uppercase tracking-wider mb-2 block">Height (cm)</label>
            <input type="number" placeholder="175" value={formData.height_cm}
              onChange={e => update("height_cm", e.target.value)}
              className="w-full bg-zinc-900 border border-zinc-700 rounded-lg px-4 py-2.5 text-sm text-white focus:outline-none focus:border-emerald-600 transition-colors" />
          </div>
          <div>
            <label className="text-zinc-500 text-xs uppercase tracking-wider mb-2 block">Weight (kg)</label>
            <input type="number" placeholder="75" value={formData.weight_kg}
        ...
        <div className="space-y-6">
          ...
          <div>
            <label className="text-zinc-500 text-xs uppercase tracking-wider mb-3 block">
              Anterior View <span className="text-zinc-600 ml-2 normal-case">Stand neutral, arms at sides</span>
            </label>
            <div onClick={() => document.getElementById("front-photo").click()}
              ...
              {photo ? (
                <img src={URL.createObjectURL(photo)} alt="front" className="max-h-48 mx-auto rounded-lg object-cover" />
              ) : (
                <div>
          ...
          <div>
            <label className="text-zinc-500 text-xs uppercase tracking-wider mb-3 block">
              Posterior View <span className="text-zinc-600 ml-2 normal-case">Same position, facing away</span>
            </label>
            <div onClick={() => document.getElementById("back-photo").click()}
              className="border border-dashed border-zinc-700 hover:border-emerald-600 rounded-xl p-8 text-center cursor-pointer transition-colors duration-150">
              {backPhoto ? (
                <img src={URL.createObjectURL(backPhoto)} alt="back" className="max-h-48 mx-auto rounded-lg object-cover" />
              ) : (
                <div>
      ...
      <div className="max-w-lg mx-auto px-6 py-10">
        <div className="mb-8">
          ...
          <div className="flex justify-between mt-2">
            {steps.map((s, i) => (
              <span key={s} className={`text-xs ${i === step ? "text-emerald-500" : i < step ? "text-zinc-500" : "text-zinc-700"}`}>{s}</span>
            ))}
          </div>
        </div>

        <div className="mb-10 min-h-[300px]">{renderStep()}</div>
